...
> **현재는 설계 v0.1입니다.** 공식 샘플 3건의 분류, 사용자 흐름, 스킬 초안을 공개했습니다. 자동 추천·이미지 생성 연결·설치형 배포는 개발 예정입니다. 아래 이미지는 **정부기관의 디자인 참고 사례**이며, 이 프로젝트가 생성한 결과물이 아닙니다.
...
다음은 **구현 목표인 출력 구성**입니다. 현재 버전에서 자동 생성되지는 않습니다.
...
- **Codex 내장 이미지 도구**를 사용하는 실행기를 연결할 계획입니다.
- 현재 채택한 연결 대상은 [baoyu-codex-imagegen](https://github.com/JimLiu/baoyu-skills/tree/main/packages/baoyu-codex-imagegen)입니다. 아직 이 저장소에 설치·연결하지 않았습니다.
...
- 스킬 초안, 출력 계약, 권리·공개 원칙 작성

**다음 단계**
1. 정부부처 샘플 확장 및 이미지별 이용조건 검토
2. 내용 구조·디자인 추천과 장별 프롬프트 생성
3. Codex 실행기 연결, 순차 생성과 중단 후 재개
4. 실제 카드 세트 검수 및 설치형 스킬 배포
